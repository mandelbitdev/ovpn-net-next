...
add_peer() {
	if [ "${PROTO}" == "UDP" ]; then
		if [ ${1} -eq 0 ]; then
			...
			ip netns exec peer${1} ${OVPN_CLI} new_peer tun${1} $((${1}+9)) ${1} \
				${LPORT} ${RADDR} ${RPORT}
			ip netns exec peer${1} ${OVPN_CLI} new_key tun${1} $((${1}+9)) 1 0 \
				${ALG} 1 data64.key
		fi
	else
		if [ ${1} -eq 0 ]; then
			...
			ip netns exec peer${1} ${OVPN_CLI} connect tun${1} $((${1}+9)) ${1} \
