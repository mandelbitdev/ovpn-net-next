...
for p in $(seq 1 ${NUM_PEERS}); do
	...
	ip netns exec peer${p} "${OVPN_CLI}" set_peer tun${p} $((${p}+ID_OFFSET)) 60 120
done
...
for p in $(seq 1 ${NUM_PEERS}); do
	...
	timeout ${NGREP_TIMEOUT} ip netns exec peer${p} ngrep -xqn 1 -X \
		${HEADER1} -S ${CAPTURE_LEN} -d veth${p} 1>/dev/null &
	NGREP_PID1=$!
	timeout ${NGREP_TIMEOUT} ip netns exec peer${p} ngrep -xqn 1 -X \
		${HEADER2} -S ${CAPTURE_LEN} -d veth${p} 1>/dev/null &
	NGREP_PID2=$!
...
for p in $(seq 1 ${NUM_PEERS}); do
	...
	ip netns exec peer${p} ${OVPN_CLI} new_key tun${p} $((${p} + ID_OFFSET)) 2 1 ${ALG} 1 data64.key
	ip netns exec peer${p} ${OVPN_CLI} swap_keys tun${p} $((${p} + ID_OFFSET))
done
...
for p in $(seq 2 ${NUM_PEERS}); do
	ip netns exec peer${p} ${OVPN_CLI} get_key tun${p} $((${p} + ${ID_OFFSET})) 1
	ip netns exec peer${p} ${OVPN_CLI} get_key tun${p} $((${p} + ${ID_OFFSET})) 2
done
...
for p in $(seq 3 ${NUM_PEERS}); do
	ip netns exec peer${p} ${OVPN_CLI} del_key tun${p} $((${p} + ${ID_OFFSET})) 1
	ip netns exec peer${p} ${OVPN_CLI} del_key tun${p} $((${p} + ${ID_OFFSET})) 2
done

echo "Setting timeout to 3s MP:"
for p in $(seq 3 ${NUM_PEERS}); do
	ip netns exec peer0 ${OVPN_CLI} set_peer tun0 ${p} 3 3 || true
	ip netns exec peer${p} ${OVPN_CLI} set_peer tun${p} $((${p} + ${ID_OFFSET})) 0 0
done
...
for p in $(seq 3 ${NUM_PEERS}); do
	ip netns exec peer${p} ${OVPN_CLI} set_peer tun${p} $((${p} + ${ID_OFFSET})) 3 3
done
sleep 5
