...
ovpn_run_basic_traffic() {
	...
	for p in $(seq 1 ${OVPN_NUM_PEERS}); do
		...
		ovpn_cmd_ok "send baseline traffic to peer ${p}" \
			...
			ping -qfc 100 -w 3 "$(ovpn_peer_vpn_addr "${p}")"
		ovpn_cmd_ok "send large-payload traffic to peer ${p}" \
			ip netns exec ovpn_peer0 \
			ping -qfc 100 -s 3000 -w 3 \
...
ovpn_run_float_mode() {
	...
	for p in $(seq 1 ${OVPN_NUM_PEERS}); do
		...
		ovpn_cmd_ok "ping tunnel after float peer ${p}" \
			ip netns exec "${peer_ns}" ping -qfc 100 -w 3 \
			"$(ovpn_peer_vpn_server_addr "${p}")"
	done
}
...
ovpn_run_iperf() {
	...
	ovpn_cmd_ok "run iperf throughput flow" \
		ip netns exec ovpn_peer1 iperf3 -Z -t 3 -c \
			"$(ovpn_peer_vpn_server_addr 1)"
...
ovpn_run_traffic_delete_peer() {
	...
	ovpn_run_bg ping_pid ip netns exec ovpn_peer2 ping -qf -w 4 \
		"$(ovpn_peer_vpn_server_addr 2)"
	sleep 2
