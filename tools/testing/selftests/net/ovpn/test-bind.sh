...
ovpn_bind_run_addr_tests() {
	...
	ovpn_run_stage "setup network topology" ovpn_bind_prepare_network
	ovpn_run_stage "peer1 bind_addr=10.10.10.1 sends from 10.10.10.1 via veth2" \
		ovpn_bind_run_addr_positive_case \
		10.10.10.1 20.20.20.2 10.10.10.1 veth2 veth1
	ovpn_run_stage "peer1 bind_addr=20.20.20.1 sends from 20.20.20.1 via veth1" \
		ovpn_bind_run_addr_positive_case \
		20.20.20.1 10.10.10.2 20.20.20.1 veth1 veth2
	ovpn_run_stage "peer2 bind_addr=10.10.10.2 accepts dst 10.10.10.2 via veth1" \
		ovpn_bind_run_addr_receiver_positive_case \
		10.10.10.2 10.10.10.2 10.10.10.1 veth1 veth2
	ovpn_run_stage "peer2 bind_addr=20.20.20.2 accepts dst 20.20.20.2 via veth2" \
		ovpn_bind_run_addr_receiver_positive_case \
